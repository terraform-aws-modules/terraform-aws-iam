...
| <a name="input_pod_identities"></a> [pod\_identities](#input\_pod\_identities) | Map of identities per cluster. Root map key is a cluster name, root values is a map of identities. Identities is a map of list(string), where key is a namespace, and value is a list of service account names in the respective namespace | `map(map(list(string)))` | `{}` | no |
