...
| <a name="input_override_trust_policy_documents"></a> [override\_trust_policy\_documents](#input\_override\_trust_policy\_documents) | List of IAM policy documents that are merged together into the trust policy. In merging, statements with non-blank `sid`s will override statements with the same `sid` | `list(string)` | `[]` | no |
...
| <a name="input_source_trust_policy_documents"></a> [source\_trust_policy\_documents](#input\_source\_trust_policy\_documents) | List of IAM policy documents that are merged together into the trust policy. Statements must have unique `sid`s | `list(string)` | `[]` | no |
