...
| <a name="input_ebs_csi_volume_tagging"></a> [ebs\_csi\_volume\_tagging](#input\_ebs\_csi\_volume\_tagging) | Determines whether to allow the EBS CSI driver to tag existing volumes/snapshots by removing the `ec2:CreateAction` condition on `ec2:CreateTags`. Required for `VolumeAttributesClass` tag modifications; disabled by default as it broadens tagging permissions | `bool` | `false` | no |
